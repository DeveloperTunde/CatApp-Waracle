...
describe("uploadStore", () => {
  ...
  describe("selectImage", () => {
    ...
    it("sets validation error for unsupported type", () => {
      ...
      expect(useUploadStore.getState().validationErrors.length).toBeGreaterThan(
        0,
      );
    });
  ...
  describe("upload", () => {
    it("returns null and sets error when no image selected", async () => {
      ...
      expect(useUploadStore.getState().error).toBe(
    ...
    it("sets error on API failure", async () => {
      ...
      (uploadCatImage as jest.Mock).mockRejectedValue(
